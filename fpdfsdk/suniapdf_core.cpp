...
void PdfCore::ScanPagePathObj(FPDF_PAGE page,double x_scale,double y_scale)
{
  ...
  for (int i = 0; i < count; i++) {
    ...
    std::cout << "FPDFPageObj_SetStrokeWidth pen_width:" << pen_width << ",suc:" << suc
              << std::endl;
    float x_h;
    float x_v;
    float p_w;
    ...
    suc = FPDFAnnot_GetColor(annotation, FPDFANNOT_COLORTYPE_Color, &R, &G, &B,
                             &A);
    suc = FPDFAnnot_SetColor(annotation, FPDFANNOT_COLORTYPE_Color, R, G, B, A,true);
    //test
    ...
    suc = (FPDF_BOOL)FPDFAnnot_GetAP(annotation, FPDF_ANNOT_APPEARANCEMODE_NORMAL,
...
**/
int PdfCore::SetAuthenticate(OfflineCertificateInfo* offline_certificate_info,const char* enc_file_path,
                int& suc_type,char modules_authority[500],bool is_string){
  int suc = -1;
  #if(CPU_TYPE < 2)
  int suc = Authenticate(offline_certificate_info,enc_file_path,suc_type,modules_authority,is_string);
  if(suc == 0)
